...
class QL:
    def __init__(self, act_num, state_num, act_range, gamma=0.1, lr=0.85, temp=1.0):
        # Some parameters
    ...
    def boltzmann_sampling(self, s_i):
        prob_list = np.exp(self.q_table[s_i] / self.T) / sum(np.exp(self.q_table[s_i] / self.T))
